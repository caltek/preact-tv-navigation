      ...
      <SpatialNavigationRoot isActive={true}>
        ...
        <header style={{
          ...
          <p style={{ margin: '0 0 10px 0', fontSize: '14px', color: '#888' }}>
            {currentData.length.toLocaleString()} items • Scroll starts at middle, keeps items centered
          </p>
          {selectedItem && (
                ...
                >
                  {({ isFocused }: FocusableNodeState) => (
                    <div style={{
                      ...
                      transition: 'all 0.2s',
                      transform: isFocused ? 'translateX(5px) scale(1.02)' : 'translateX(0)',
                      boxShadow: isFocused ? '0 4px 15px rgba(233, 30, 99, 0.5)' : 'none',
                    }}>
        ...
        <footer style={{
          ...
          ↕️ Navigate list items • ↔️ Switch between sidebar and list • Press Enter to select
        </footer>
      </div>
      </SpatialNavigationRoot>
    </SpatialNavigationDeviceTypeProvider>
...
function DatasetButton({ label, icon, isActive, onClick }: {
  label: string;
  icon: string;
  isActive: boolean;
  onClick: () => void;
